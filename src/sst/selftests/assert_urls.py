...
from sst.tests import DEVSERVER_PORT

from urlparse import urlparse


# haven't visited a url yet, so all assert_url* fail
fails(assert_url, 'http://localhost:8120/')
...
assert_url('/begin/')
assert_url('begin/')
assert_url('http://localhost:8120/begin')
assert_url('http://localhost:8120/begin/')
assert_url(urlparse('http://wrongurl/begin').path)
fails(assert_url, 'http://wrongurl/begin')

assert_url_contains('http://localhost:8120/begin')
assert_url_contains('localhost:8120')
...
assert_url_network_location('localhost:8120')
fails(assert_url_network_location, 'localhost')
...
assert_url('http://localhost:8120/begin?query_string#fragment_id')
assert_url('/begin?query_string#fragment_id')
fails(assert_url, '/begin')
fails(assert_url, 'http://localhost:8120/begin')
